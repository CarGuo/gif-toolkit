...
| **R-83** | toolboxBudgetCompress 必须绕开 `DEFAULT_OPTIONS.minSize=450`,改用 `ABSOLUTE_MIN_SIDE=200`;size-regression 时 `fsp.copyFile` 回退原图 + emit `substep:'size-regression-reverted'` + `sizeRegression.reverted=true` | [R-83](file:///Users/guoshuyu/workspace/gif-toolkit/harness/rules/R-83-toolbox-budget-ignores-minsize.md) |
| **R-84** | 所有 yt-dlp 调用必须带默认 Chrome UA;对 `bilibili.com` / `b23.tv` 额外注入 `Referer: https://www.bilibili.com`,否则触发 HTTP 412(`resolveDirectUrl` 走 ytdlp-nodejs 路径为已知后续项) | [R-84](file:///Users/guoshuyu/workspace/gif-toolkit/harness/rules/R-84-ytdlp-default-headers.md) |
| **R-85** | gif-optimize 三分支选路顺序固定为 `hasBudget > hasMethod > hasExplicit > fallthrough`:用户给了字节预算,永远进 budget 分支,不被 method picker 截胡 | [R-85](file:///Users/guoshuyu/workspace/gif-toolkit/harness/rules/R-85-hasBudget-dominates-method-picker.md) |
...
| **R-88** | `useToolbox.setKind` per-kind sticky:`paramsByKindRef` 缓存每个 kind 的最近参数,切走再切回优先恢复,而非每次 fallback `defaultParamsFor`;`setParams` / `applyPreset` 必须镜像写回 cache | [R-88](file:///Users/guoshuyu/workspace/gif-toolkit/harness/rules/R-88-toolbox-paramsByKind-sticky.md) |
...
| **R-COMPRESS-V2** | compressLoop / gifsicleMethod 5 件 review 修复:C-01 adaptiveStartLossy 唯一 source(只读 processor-utils);C-02 lossySearch 对称早退(decideEarlyAccept 三返);C-03 'color-reduction' picker 永远 `--no-dither`(命名即契约);C-04 Phase C/D 必须从 ORIGINAL inputGif 解码(避免双重 quantise);C-05 recordBest 三档 tier + same-tier smaller-wins(shouldReplaceBest) | [R-COMPRESS-V2](file:///Users/guoshuyu/workspace/gif-toolkit/harness/rules/R-COMPRESS-V2-loop-coherence.md) |
...
| **R-REC-DESKTOP-AREA** | 桌面区域录屏 → GIF。主进程独占 ffmpeg(avfoundation/gdigrab/x11grab),renderer 不直接抓帧;transparent BrowserWindow + 拖框选区;buildRecorderArgs 抽纯函数 + 10 case 跨平台契约锁;mac 必须探测屏幕录制权限 + 深链系统设置;cancel 走 stdin `q\n` + 2s SIGKILL 兜底;overlay preload 最小化(3 个方法);录完 mp4 复用 toolbox video-to-gif 链路不重复造；**v2.1 起 #probe-device / #even-pixel / #loglevel-warning：mac 必须 detectMacScreenDevice() 5min cache 替代硬编 -i 1 / region.w/h 必须 toEvenSize 防 libx264 yuv420p 奇数 throw / -loglevel 必须 warning 透 init 致命错** | [R-REC-DESKTOP-AREA](file:///Users/guoshuyu/workspace/gif-toolkit/harness/rules/R-REC-DESKTOP-AREA-recorder.md) |
| **R-DOCK-FLOATING** | 桌面悬浮控件（floating dock）。专用 BrowserWindow + 专用 [preload/dockOverlay.ts](file:///Users/guoshuyu/workspace/gif-toolkit/src/preload/dockOverlay.ts) 白名单 8 方法（v2 +getRecorderState/+revealLastRecording/+onRecorderState 订阅）；v2 起 dock **自治录屏链路**：dispatchDockAction 11 个 case（+dock-record-region/stop/cancel/open-output-dir，tone='danger' 给 stop），主进程直接调 openRegionSelectorOverlay + startRecorder + showStaticOverlayForRegion（录制全程只读高亮 + 点击穿透），mac 上 `avfoundationDeviceIndex` 必须兜底 1，recorder:progress 必须 fan-out 给 mainWindow + dock；before-quit 必须 destroyDockWindow + cancelRecorder + closeStaticOverlay；**v2.1 起 #error-toast：error 阶段圆球不许全红/大 `!`/脉动，只在右上角加 10×10 mini-dot；错误内容走 dock 内底部独立 toast 条（DOCK_ERROR_SIZE 扩高 + position:absolute + ✕ 立即关闭走 dock-record-cancel 转重置）** | [R-DOCK-FLOATING](file:///Users/guoshuyu/workspace/gif-toolkit/harness/rules/R-DOCK-FLOATING-floating-dock.md) |
