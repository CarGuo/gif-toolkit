...
import type { SniffResult, SniffedMedia, MediaKind } from '../shared/types';
...
function collectFromDom(
  ...
  $('img').each((_, el) => {
    ...
    const kind: MediaKind = GIF_EXTS.has(ext) ? 'gif' : 'image';
 ...
 */
function importHtmlFile(absHtmlPath: string): SniffResult {
  const baseDir = path.dirname(absHtmlPath);
  const html = fs.readFileSync(absHtmlPath, 'utf8');
  const $ = cheerio.load(html);
  const pageUrl = pathToFileURL(absHtmlPath).toString();
  const map = new Map<string, SniffedMedia>();
  const title = collectFromDom($, baseDir, pageUrl, map);
  const warnings: string[] = [];
  if (map.size === 0) {
    warnings.push(
      '页面里没有找到 <video>/<img>/og:video/og:image 引用,' +
 ...
 */
function importMhtmlFile(absPath: string): SniffResult {
  ...
  const rewriteAttr = (sel: string, attr: string): void => {
    $(sel).each((_, el) => {
      ...
      if (staged) {
        $(el).attr(attr, pathToFileURL(staged).toString());
      }
    });
  };
  ...
  const map = new Map<string, SniffedMedia>();
  const pageUrl = primaryHtmlOrigin || pathToFileURL(absPath).toString();
  ...
  const title = collectFromDom($, stagedDir, pageUrl, map);
  const warnings: string[] = [];
  if (map.size === 0) {
    warnings.push('mhtml 解析成功,但里面没有找到 <video>/<img>/og:video/og:image。');
  }
  return {
    pageUrl,
    title,
 ...
 */
export async function importOfflinePath(absPath: string): Promise<SniffResult> {
  ...
  if (st.isDirectory()) {
    ...
    return importHtmlFile(path.join(absPath, htmlEntry));
  }
  const ext = path.extname(absPath).toLowerCase();
  if (MHTML_EXTS.has(ext)) return importMhtmlFile(absPath);
  if (HTML_EXTS.has(ext)) return importHtmlFile(absPath);
  if (classifyByExt(absPath)) return importSingleMediaFile(absPath);
