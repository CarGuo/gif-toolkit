...
- No **不允许**`sharp(...)` 调用不传 `limitInputPixels: false`(默认 268MP 上限对动图太低)。
- No **不允许**sharp 失败后直接 throw 给 compressLoop(必须 ffmpeg 兜底,否则 R-04 全 phase 失败)。
- No **不允许**sharp fallback 吞掉 `CancelledError`(取消信号必须穿透到外层任务调度)。
- No **不允许**ffmpeg 兜底的 `-vf scale` 用奇数宽度(部分编码器 / 滤镜要求偶数,用 `Math.floor(W/2)*2` 或 `-2` 占位)。
