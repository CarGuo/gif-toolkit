...
- **打包阶段** Yes yt-dlp 二进制必须随 dmg / installer 一起分发：`electron-builder.asarUnpack` 必须包含 `node_modules/ytdlp-nodejs/bin/**`，不允许在 `build.files` 排除该路径
- **启动阶段** Yes `checkYtdlp()` 仅做诊断（fs.stat 优先级目录），不联网；`source` 在生产环境必须命中 `'packaged'`
- **嗅探完成后** Yes 渲染端 `useEffect` 必须自动遍历 `requiresExternalDownload` 的 items，调 `resolveEmbed` 批量预解析，无需用户交互
- **运行时解析** Yes resolver 内部按四级优先级 fallback 找 binary：
...
- **失败兜底** Yes resolver 失败时**保留 embed 卡片**，仅在卡片右下显示 `↻ 重试解析` 小按钮 + redact 过的 error tooltip；`resolveErrorMap` 防止 useEffect 循环触发
- **不存在的概念** No 渲染端**不允许**有任何 `confirm()` 弹窗、`installYtdlp` / `uninstallYtdlp` IPC、`ytdlp-chip` 状态徽章；preload 不暴露这些 API；main 进程也不再注册 `resolve:installYtdlp` / `resolve:uninstallYtdlp` handler
...
- No 在 titlebar 加 ` yt-dlp 未装` chip，要求用户感知二进制存在
- No 在 MediaGrid 卡片上加橙色 ` 解析直链` 按钮，要求用户主动点
- No 用 `confirm()` 弹窗询问 "是否下载 yt-dlp 30MB"
- No 让 yt-dlp 二进制不打包，用户首次解析时联网下载（违背开箱即用）
- No resolver header 沿用 yt-dlp extractor 全量返回（包含 Authorization / Set-Cookie / Host 等敏感字段）
- No 自动解析没有 `resolveErrorMap` 守卫，导致 useEffect 在每次失败后无限循环
