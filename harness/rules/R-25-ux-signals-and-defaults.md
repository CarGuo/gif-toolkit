...
4. **#4 默认值**:[DEFAULT_OPTIONS](file:///Users/guoshuyu/workspace/gif-toolkit/src/shared/types/) `minSize` **必须**为 450(原 240),`concurrency` **必须**为 3(原本就是 3,显式锁定)。**反向**:不允许把 `minSize` 改回 240(R-03 + R-06 仍然成立,但用户认为 240 太糊),不允许把 `concurrency` 设为 `undefined` 让 main 默认接管(必须 renderer 显式给值,IPC payload 才有审计性)。
...
- [src/shared/types/ DEFAULT_OPTIONS](file:///Users/guoshuyu/workspace/gif-toolkit/src/shared/types/)
