...
+----------------+-----------------+
                 | window.giftk.db.* (preload-exposed IPC)
                 v
+----------------------------------+
| Main process                     |
...
> **history schema decision (R-80 implementation note)**: useHistory 的
> `HistoryRecord` 内层是 `SniffedMedia[]`(每个含 `resolved` 嵌套 + 多个
> `Record<string, T>` map),这些字段都不参与查询/排序,真去拍 history_jobs
> 子表会在子表里再塞 7 个 JSON 列,得不偿失。最终采取折中:**仅**外层
> 排序/过滤字段(`page_url`, `title`, `created_at`)上列,所有深嵌套统一用
> JSON 列。upload/sniff/toolbox 三张表字段相对扁平,按关系列拍平。

```sql
-- Schema version metadata; one row per logical table family, updated
-- by the migrations runner. v0 means "fresh install, no schema yet".
CREATE TABLE IF NOT EXISTS schema_meta (
  k TEXT PRIMARY KEY,
  v INTEGER NOT NULL
);

-- Processing-history sessions. Outer columns are query/sort axes;
-- everything that is only ever displayed lives in JSON columns.
CREATE TABLE IF NOT EXISTS history (
  ...
  title TEXT,
  output_dir TEXT,
  items_json TEXT NOT NULL,             -- SniffedMedia[]
  options_json TEXT NOT NULL,           -- ProcessOptions snapshot
  outputs_json TEXT NOT NULL DEFAULT '{}', -- outputsByTaskId
  status_json TEXT NOT NULL DEFAULT '{}',  -- taskStatus
  uploads_json TEXT NOT NULL DEFAULT '{}'  -- uploadsByOutputPath (R-54)
);
...
-- Upload-history sessions are tier-1 flat: outer record + per-row item.
...
CREATE TABLE IF NOT EXISTS upload_history_items (
  ...
  file_hash TEXT,
  reused INTEGER,        -- 0/1
  percent INTEGER,       -- 0..100, NULL when unknown
  position INTEGER NOT NULL  -- preserves insertion order within record
);
...
-- Sniff history is tiny — one row per URL.
...
CREATE TABLE IF NOT EXISTS toolbox_history (
  ...
  outputs_json TEXT NOT NULL,    -- string[]
...
```ts
// src/preload/index.ts (additions to the existing `giftk` namespace)
contextBridge.exposeInMainWorld('giftk', {
  // ...existing channels
  db: {
    history: {
